...
builtin_state :: State 
builtin_state = State [] [Tdef "Int" (str_to_type "Int"), Tdef "Player" (str_to_type "Player"), Tdef "Board" (str_to_type "Board"), Tdef "Empty" (str_to_type "empty")]
...
ast_convert_b s1 (Btype s2) d (State fs ts) = if(lookup1 ts s2)
                                                then (ast_func_convert (type_lookup ts s2) ts, d, State fs ts, True, "")
                                                        ...
                                                        else let ((Tdef s3 t2):dfs) = lookup2 d s2
